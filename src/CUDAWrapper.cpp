...
#include <stdio.h>
__global__ void setup_kernel(curandState *state, int iterations, int total_threads, int seed, int device_id)
{
...
void CUDAWrapper::curand_setup(int num_blocks, int threads_per_block, int total_threads, void** states, int seed, int device_id) {
	...
	setup_kernel<<<num_blocks, threads_per_block>>>((curandState*) *states, iterations, total_threads, seed, device_id);
}
